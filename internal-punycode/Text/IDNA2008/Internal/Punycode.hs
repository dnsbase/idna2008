...
-- Stability   : unstable
--
-- A self-contained implementation of RFC 3492 Punycode used by IDNA
-- A-label \(\Leftrightarrow\) U-label conversion.  The encoder and decoder
-- both write into caller-supplied mutable buffers; no intermediate
-- 'ByteString' or 'String' is allocated, and arithmetic is performed in
-- machine 'Int' with explicit overflow checks.
--
-- Inputs:
--
--   * The encoder consumes a 'PrimArray' of codepoint values (each in
--     @0..0x10FFFF@, surrogates excluded -- the caller is responsible for
--     enforcing that) and writes the corresponding base-36 ACE bytes
--     into a 'MutableByteArray'.  The output does /not/ include the
--     @\"xn--\"@ prefix; callers add that.
--
--   * The decoder consumes a slice of an immutable 'ByteArray' (the
--     ACE body, /not/ including @\"xn--\"@) and writes the recovered
--     codepoints into a 'MutablePrimArray'.
--
...
-- | Errors that can be produced by the encoder or decoder.  All inputs that
-- correspond to a label of bounded length (\(\le\) 63 wire octets) over a
-- valid Unicode codepoint range are guaranteed not to overflow on a 64-bit
-- 'Int'; the overflow check exists primarily to keep the routines safe under
...
-- | Decode a Punycode ASCII body (no @\"xn--\"@ prefix) into a sequence
-- of codepoints, written into the caller-supplied mutable codepoint
-- buffer.  Returns the number of codepoints produced.
--
-- The decoder accepts both upper- and lower-case base-36 digits;
-- callers performing strict A-label round-trip checks should compare the
-- /lower-cased/ input ACE bytes against a freshly encoded form.
punycodeDecode
    :: forall s
