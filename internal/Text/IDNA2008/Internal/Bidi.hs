...
newtype BidiClass = BidiClass Word8 deriving Eq
...
pattern BidiL     = BidiClass 0     -- ^ Left_To_Right
pattern BidiR     :: BidiClass
pattern BidiR     = BidiClass 1     -- ^ Right_To_Left
pattern BidiAL    :: BidiClass
pattern BidiAL    = BidiClass 2     -- ^ Arabic_Letter
pattern BidiAN    :: BidiClass
pattern BidiAN    = BidiClass 3     -- ^ Arabic_Number
pattern BidiEN    :: BidiClass
pattern BidiEN    = BidiClass 4     -- ^ European_Number
pattern BidiNSM   :: BidiClass
pattern BidiNSM   = BidiClass 10    -- ^ Nonspacing_Mark
pattern BidiOther :: BidiClass
pattern BidiOther = BidiClass 11    -- ^ everything else: B, S, WS,
...
newtype BidiSummary = BidiSummary Word16 deriving Eq
...
bsFirstClass (BidiSummary w) = BidiClass (fromIntegral (w .&. 0x000F))
{-# INLINE bsFirstClass #-}
...
bsLastNonNSM (BidiSummary w) =
    BidiClass (fromIntegral ((w `unsafeShiftR` 4) .&. 0x000F))
{-# INLINE bsLastNonNSM #-}

bsHasL, bsHasRorAL, bsHasAN, bsHasEN, bsHasOther :: BidiSummary -> Bool
bsHasL     (BidiSummary w) = testBit w bsHasLIdx
bsHasRorAL (BidiSummary w) = testBit w bsHasRorALIdx
bsHasAN    (BidiSummary w) = testBit w bsHasANIdx
bsHasEN    (BidiSummary w) = testBit w bsHasENIdx
bsHasOther (BidiSummary w) = testBit w bsHasOtherIdx
{-# INLINE bsHasL #-}
{-# INLINE bsHasRorAL #-}
{-# INLINE bsHasAN #-}
{-# INLINE bsHasEN #-}
...
addBidiTrigger !trig (BidiSummary w) =
...
emptyBidiSummary = BidiSummary 0x00BB
...
extendBidiSummary !(BidiSummary w) !c@(BidiClass cw) =
...
--     label of a domain once any label has Bidi-flavoured
--     content.
--
