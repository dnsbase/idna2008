...
-- Description : Generic CLI-token vocabulary for bit-flag types.
...
-- (multi-bit aliases like @host@ for 'LabelFormSet' or @map@ for
-- 'Text.IDNA2008.Internal.Flags.IdnaFlags').  The functions in
-- this module turn that data into a parser and a renderer.
--
...
-- matches require at least 'minPrefixLen' characters of input
-- so that short shortcuts that happen to be unambiguous today
-- don't quietly break when a future release adds a collision.
--
...
parseTokens !table !presets !implicitBase !withoutOp !input =
  ...
  where
    ...
    -- prefix.  Three characters is enough for all the
    -- pith-flavoured prefixes we want today (@asc@, @bid@,
    -- @att@, ...) while leaving room for future tokens to share
    -- a 1- or 2-letter prefix without breaking existing scripts.
    minPrefixLen :: Int
    minPrefixLen = 3
...
-- 'parseTokens'.
--
-- Provided so CLI consumers can compose @'asciiByteString'
-- '>=>' parseLabelFormSet@ (or the like) and get a precise
-- diagnostic at the input boundary.  By the time
-- @optparse-applicative@ hands a String to a custom reader,
-- GHC's runtime has already decoded the OS-level @argv@ bytes
-- through the file-system encoding (UTF-8 with surrogate-escape
-- on modern systems), so non-ASCII codepoints in the input may
-- represent either genuine UTF-8 input or invalid bytes
-- escaped as 'Char's in the surrogate range; this helper
-- rejects both with the same error rather than letting them
-- slip through a lossy 'Data.ByteString.Char8.pack'.
